...
Tuig already has a compact colored commit graph with merge edges, branch labels, and tags; commit metadata and diffs; a changed-file tree with staging and commits; fetch/pull/push shortcuts; stash create/apply/pop/drop; worktree add and remove; cherry-pick, reset, and rebase with confirmation dialogs; commit tagging; and author avatars. The work below tracks the remaining path beyond that MVP.

## Finish the core workflow

- Extend right-click menus from graph, branch, and stash rows to worktree and changed-file rows.
- Add a worktree lock/unlock action and per-file discard.
