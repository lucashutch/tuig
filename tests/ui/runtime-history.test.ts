...
describe("graph row hit testing", () => {
  test("maps the dot line and its continuation line to the same commit", () => {
    // No working changes: body line 0 is the first commit's dot line.
    expect(commitRowAtLine(0, 0, false)).toBe(0);
    expect(commitRowAtLine(1, 0, false)).toBe(0);
    expect(commitRowAtLine(2, 0, false)).toBe(1);
    expect(commitRowAtLine(3, 0, false)).toBe(1);
  });
  ...
  test("skips the working row when it is on screen", () => {
    ...
    expect(commitRowAtLine(2, 0, true)).toBe(0);
    expect(commitRowAtLine(3, 0, true)).toBe(1);
  });
  ...
  test("keeps scrolled viewports aligned with their first commit", () => {
    ...
    expect(commitRowAtLine(1, 1, true)).toBe(0);
    expect(commitRowAtLine(2, 1, true)).toBe(1);
    expect(commitRowAtLine(0, 5, false)).toBe(5);
    expect(commitRowAtLine(3, 5, false)).toBe(6);
  });
