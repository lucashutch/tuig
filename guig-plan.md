...
- Testing: `cd guig && bun install && bun run typecheck` passes; `bunx tsc --noEmit` clean.
...
- Testing: `bun run typecheck`, manual `bun run dev` shows shell with mocked API.
...
- Testing: `bun run typecheck`, graph unit check via mocked commits.
...
- Testing: `bun run typecheck`, mocked stage/commit flow.
...
- Final validation: `cd guig && bun run typecheck && bun test` (if tests added), root `bun run check` still passes for TUI (guig excluded via ignore or passes lint).
